            ...
            <div class="card-body">
                ...
                <div data-simplebar="" style="max-height: 419px;">
                    <div class="timeline-alt pb-0">
                        ...
                        <h1>{{ $novel['data'][0]['title'] ?? 'Không thể lấy truyện' }}</h1>
                        <p>{{ $novel['data'][0]['description'] ?? 'Mô tả không có sẵn' }}</p>
                        <p><strong>Tác giả:</strong> {{ $novel['data'][0]['author'] ?? 'Không có thông tin tác giả' }}
                        </p>

                        <h2>Các chương</h2>
                        @if(!empty($novel['data'][0]['chapters']))
                        <ul>
                            @foreach($novel['data'][0]['chapters'] as $chapter)
                            <li>{{ $chapter['name'] }}</li>
                            @endforeach
                        </ul>
                        @else
